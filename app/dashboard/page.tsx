...
//   const map: Record<Agent["status"], { color: string; label: string; Icon: React.FC<{ size?: number }> }> = {
//     initializing: { color: "#93c5fd", label: "Initializing", Icon: ({ size }) => <Loader size={size} style={{ animation: "spin 1s linear infinite" }} /> },
//     funded:       { color: "#facc15", label: "Funded",       Icon: ({ size }) => <Wallet size={size} /> },
//     running:      { color: "#4ade80", label: "Running",      Icon: ({ size }) => <Activity size={size} /> },
//     idle:         { color: "#94a3b8", label: "Idle",         Icon: ({ size }) => <Minus size={size} /> },
//     error:        { color: "#f87171", label: "Error",        Icon: ({ size }) => <AlertTriangle size={size} /> },
//   }
...
//   const map = {
//     BUY:  { bg: "rgba(74,222,128,0.12)",  color: "#4ade80", Icon: ArrowUpRight },
//     SELL: { bg: "rgba(248,113,113,0.12)", color: "#f87171", Icon: ArrowDownRight },
//     HOLD: { bg: "rgba(148,163,184,0.12)", color: "#94a3b8", Icon: Minus },
//   }
...
// // ── StatCard ──────────────────────────────────────────────────────
// function StatCard({
//   label, value, sub, accent, loading, icon: Icon,
// }: {
//   label: string
//   value: string
//   sub?: string
...
//         <p style={{
//           color: "var(--muted-foreground)", fontSize: "0.72rem",
...
//         color: loading ? "var(--border)" : accent ? "var(--primary)" : "var(--foreground)",
//         lineHeight: 1, transition: "color 0.3s",
//       }}>
//         {loading ? "—" : value}
//       </p>
//       {sub && (
//         <p style={{ color: "var(--muted-foreground)", fontSize: "0.75rem", marginTop: 4 }}>
//           {sub}
//         </p>
//       )}
//     </div>
...
// // ── AgentCard ─────────────────────────────────────────────────────
// function AgentCard({
//   agent, onTrade, onAirdrop, onViewHistory, trading, dropping,
// }: {
//   agent: Agent
...
//     <div style={{
//       background: "var(--card)",
//       border: `1px solid ${agent.status === "running" ? "rgba(74,222,128,0.25)" : "var(--border)"}`,
...
//           </p>
//           <p style={{ color: "var(--muted-foreground)", fontSize: "0.72rem" }}>SOL</p>
//         </div>
//       </div>

//       {/* Public key row */}
//       <div style={{
//         background: "var(--muted)", borderRadius: 8, padding: "0.6rem 0.85rem",
//         display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8,
//       }}>
//         <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
//           <Key size={11} color="var(--muted-foreground)" />
//           <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.72rem", color: "var(--muted-foreground)" }}>
...
//           style={{ display: "inline-flex", alignItems: "center", gap: 3, color: "var(--primary)", fontSize: "0.72rem", textDecoration: "none" }}
//         >
...
//             Icon: <Activity size={11} color="var(--muted-foreground)" />,
//           },
//           {
//             label: "P&L",
//             value: `${agent.pnl >= 0 ? "+" : ""}${agent.pnl.toFixed(4)}`,
//             color: agent.pnl >= 0 ? "#4ade80" : "#f87171",
//             Icon: agent.pnl >= 0
...
//             Icon: <Clock size={11} color="var(--muted-foreground)" />,
//           },
//         ].map(({ label, value, color, Icon }) => (
//           <div key={label} style={{
//             background: "var(--muted)", borderRadius: 8, padding: "0.6rem",
//             textAlign: "center",
//           }}>
//             <div style={{ display: "flex", justifyContent: "center", marginBottom: 3 }}>{Icon}</div>
//             <p style={{ color: "var(--muted-foreground)", fontSize: "0.62rem", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 2 }}>
//               {label}
//             </p>
//             <p style={{ color, fontSize: "0.82rem", fontWeight: 500, fontFamily: "var(--font-mono)" }}>
//               {value}
//             </p>
//           </div>
//         ))}
...
//             <p style={{ color: "var(--muted-foreground)", fontSize: "0.75rem", lineHeight: 1.5 }}>
...
//                 target="_blank" rel="noopener noreferrer"
//                 style={{ display: "inline-flex", alignItems: "center", gap: 3, color: "var(--primary)", fontSize: "0.7rem", fontFamily: "var(--font-mono)", textDecoration: "none", marginTop: 4 }}
//               >
//                 {agent.lastTrade.signature.slice(0, 16)}… <ExternalLink size={10} />
//               </a>
//             )}
//           </div>
//         </div>
...
//           background: "rgba(248,113,113,0.08)", padding: "0.6rem 0.75rem",
//           borderRadius: 8, border: "1px solid rgba(248,113,113,0.2)",
//         }}>
//           <AlertTriangle size={13} color="#f87171" style={{ flexShrink: 0, marginTop: 1 }} />
//           <p style={{ color: "#f87171", fontSize: "0.75rem", lineHeight: 1.5 }}>{agent.error}</p>
//         </div>
//       )}
...
//           const colors = {
//             BUY:  { bg: "rgba(74,222,128,0.12)",  color: "#4ade80",  border: "rgba(74,222,128,0.25)" },
//             SELL: { bg: "rgba(248,113,113,0.12)", color: "#f87171",  border: "rgba(248,113,113,0.25)" },
//             HOLD: { bg: "var(--muted)",           color: "var(--muted-foreground)", border: "var(--border)" },
//           }
//           const c = colors[type]
//           return (
//             <button
...
//                 opacity: trading ? 0.5 : 1, transition: "opacity 0.2s",
...
//             opacity: dropping ? 0.5 : 1, transition: "opacity 0.2s",
//           }}
//         >
//           {dropping
...
//                 On-chain History
//               </p>
//               <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.7rem", color: "var(--muted-foreground)" }}>
...
//           {loading ? (
//             <div style={{ padding: "3rem", textAlign: "center", color: "var(--muted-foreground)", display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
//               <Loader size={20} style={{ animation: "spin 0.8s linear infinite" }} />
//               Fetching from devnet…
//             </div>
//           ) : txs.length === 0 ? (
//             <div style={{ padding: "3rem", textAlign: "center", color: "var(--muted-foreground)", display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
//               <Activity size={20} />
...
//                       color: tx.err ? "#f87171" : "var(--primary)", textDecoration: "none",
//                     }}
//                   >
...
//                       <p style={{ color: "#f87171", fontSize: "0.68rem" }}>Failed</p>
//                     </div>
//                   )}
//                 </div>
...
//                       <Clock size={10} color="var(--border)" />
//                       <p style={{ color: "var(--border)", fontSize: "0.68rem" }}>
...
//         {toasts.map((t) => (
//           <div key={t.id} style={{
//             background: t.type === "success" ? "rgba(74,222,128,0.12)" : t.type === "error" ? "rgba(248,113,113,0.12)" : "var(--card)",
//             border: `1px solid ${t.type === "success" ? "rgba(74,222,128,0.3)" : t.type === "error" ? "rgba(248,113,113,0.3)" : "var(--border)"}`,
//             color: t.type === "success" ? "#4ade80" : t.type === "error" ? "#f87171" : "var(--foreground)",
//             borderRadius: 12, padding: "0.75rem 1rem",
//             fontSize: "0.82rem", maxWidth: 340,
//             display: "flex", flexDirection: "column", gap: 4,
//             animation: "slideUp 0.3s ease forwards",
//           }}>
//             <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
//               {t.type === "success" && <CheckCircle size={13} />}
//               {t.type === "error"   && <AlertTriangle size={13} />}
//               {t.type === "info"    && <Info size={13} />}
//               {t.message}
//             </div>
//             {t.url && (
//               <a href={t.url} target="_blank" rel="noopener noreferrer"
//                 style={{ display: "inline-flex", alignItems: "center", gap: 4, color: "var(--primary)", fontSize: "0.72rem", textDecoration: "none", marginLeft: 20 }}>
...
//       <main style={{ maxWidth: "1200px", margin: "0 auto", padding: "7rem 1.5rem 5rem" }}>
...
//               letterSpacing: "-0.03em", color: "var(--foreground)", marginBottom: "0.4rem",
//             }}>
//               Live Agent Dashboard
//             </h1>
//             <div style={{ display: "flex", alignItems: "center", gap: 6, color: "var(--muted-foreground)", fontSize: "0.88rem" }}>
//               <Wifi size={13} />
//               Real wallets · Real transactions · Solana devnet
//               {lastRefresh && (
//                 <span style={{ marginLeft: 6, color: "var(--border)", display: "inline-flex", alignItems: "center", gap: 4 }}>
//                   · <Clock size={11} /> {timeAgo(lastRefresh.toISOString())}
//                 </span>
//               )}
//             </div>
//           </div>
...
//                 color: "var(--muted-foreground)", borderRadius: 8, cursor: "pointer",
...
//               <CircleDot
//                 size={12}
//                 color={isInit ? "#4ade80" : isIniting ? "#facc15" : "#94a3b8"}
//                 style={{ filter: isInit ? "drop-shadow(0 0 4px #4ade80)" : "none" }}
//               />
...
//           }}>
//             <Loader size={15} color="#facc15" style={{ animation: "spin 0.8s linear infinite", flexShrink: 0, marginTop: 2 }} />
//             <div>
//               <p style={{ color: "#facc15", fontWeight: 600, fontSize: "0.875rem", marginBottom: 3 }}>
...
//               </p>
//             </div>
//             <p style={{ color: "var(--muted-foreground)", fontSize: "0.82rem", marginBottom: "0.75rem", lineHeight: 1.6 }}>
//               Wallets are created and ready — they just need SOL. Copy an address below and paste it at the faucet:
//             </p>
//             <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: "0.85rem" }}>
//               {agents.map((a) => (
//                 <div key={a.id} style={{
//                   display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap",
//                   background: "var(--muted)", borderRadius: 8, padding: "0.5rem 0.85rem",
//                 }}>
//                   <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
//                     <Zap size={11} color="var(--primary)" />
//                     <span style={{ color: "var(--primary)", fontFamily: "var(--font-mono)", fontSize: "0.72rem", minWidth: 120 }}>{a.id}</span>
//                   </div>
//                   <span style={{ color: "var(--foreground)", fontFamily: "var(--font-mono)", fontSize: "0.72rem", flex: 1 }}>{a.publicKey}</span>
...
//                 }}
//               >
//                 <ExternalLink size={12} /> Open faucet.solana.com
//               </a>
//               <span style={{ color: "var(--muted-foreground)", fontSize: "0.78rem", display: "inline-flex", alignItems: "center", gap: 5 }}>
//                 or press <Fuel size={11} /> on an agent card to retry
//               </span>
//             </div>
//           </div>
//         )}

//         {/* Partial-unfunded banner */}
...
//             color: "#facc15", fontSize: "0.82rem",
//           }}>
//             <Info size={13} style={{ flexShrink: 0 }} />
//             Some agents are unfunded. Press <Fuel size={11} /> on a card to retry, or visit{" "}
...
//           letterSpacing: "-0.02em", color: "var(--foreground)", marginBottom: "1rem",
//           display: "flex", alignItems: "center", gap: 8,
//         }}>
//           <Wallet size={18} color="var(--primary)" /> Agent Wallets
//         </h2>
...
//             color: "var(--muted-foreground)", display: "flex", flexDirection: "column", alignItems: "center", gap: 12,
//           }}>
...
//             display: "flex", alignItems: "center", gap: 8,
//           }}>
//             <Info size={16} color="var(--primary)" /> How this works
//           </h2>
//           <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "1.25rem" }}>
//             {[
//               { Icon: Key,          title: "Real keypairs",     body: "Each agent generates a real ed25519 keypair on startup. Private keys are AES-256-GCM encrypted in the server store." },
//               { Icon: Fuel,         title: "Real airdrops",     body: "The fuel button hits Solana's devnet faucet and requests 1 SOL for that agent's wallet — confirmed on-chain." },
//               { Icon: ArrowUpRight, title: "Real transactions", body: "BUY sends a real SOL transfer from the agent wallet to a devnet program. Every signature is verifiable on Explorer." },
//               { Icon: Wifi,         title: "Live from RPC",     body: "Balances and tx history are fetched directly from api.devnet.solana.com — not cached or simulated." },
//             ].map(({ Icon, title, body }) => (
//               <div key={title}>
//                 <div style={{ display: "inline-flex", padding: "0.5rem", background: "var(--muted)", borderRadius: 8, marginBottom: 10 }}>
//                   <Icon size={16} color="var(--primary)" />
//                 </div>
//                 <p style={{ color: "var(--foreground)", fontSize: "0.875rem", fontWeight: 500, marginBottom: 4 }}>{title}</p>
//                 <p style={{ color: "var(--muted-foreground)", fontSize: "0.8rem", lineHeight: 1.65 }}>{body}</p>
//               </div>
//             ))}
//           </div>
//         </div>
