      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
        {[
          ...
          <div key={label} style={{
            ...
            <p style={{ color: "rgba(255,255,255,0.2)", fontSize: "0.58rem", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 3 }}>
              {label}
            </p>
            <p style={{ color, fontSize: "0.78rem", fontWeight: 700 }}>{value}</p>
          </div>
        ))}
      </div>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {Boolean(tx.err) && (
                    <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 3 }}>
                      ...
                      <p style={{ color: "#f87171", fontSize: "0.65rem", fontFamily: "'Space Mono', monospace" }}>
                        Failed
                      </p>
                    </div>
                  )}
...
export default function DashboardPage() {
  ...
  // ── Auto-start all funded agents on first initialization ──────────
  const autoStartLoops = useCallback(async (agents: Agent[]) => {
    ...
    if (funded.length === 0) {
      // No funded agents yet — reset flag so we retry on the next poll
    ...
    await Promise.all(
      funded.map(async (a) => {
        try {
          ...
          // individual agent failure — skip silently
        }
      })
    )
    ...
    if (started.length > 0) {
      ...
      addToast(
        `${started.length} agent${started.length > 1 ? "s" : ""} started trading autonomously`,
        "success"
      )
    }
  }, [addToast])

  // ── Polling: 3 s while loops active, 8 s otherwise ────────────────
  useEffect(() => {
  ...
  const handleTrade = useCallback(async (agentId: string, type: "BUY" | "SELL" | "HOLD") => {
    ...
    try {
      ...
      addToast(
        `${type} executed — ${json.newBalanceSOL?.toFixed(4)} SOL remaining`,
        "success",
        json.explorerUrl ?? undefined
      )
      await fetchAgents()
    } catch (err) {
  ...
  // ── Airdrop: faucet first → fallback to master wallet ────────────
  const handleAirdrop = useCallback(async (agentId: string) => {
    setDroppingAgent(agentId)
    type FundResult = { ok: boolean; newBalanceSOL?: number; explorerUrl?: string; error?: string; source?: string }

    // Step 1 — try devnet faucet
    try {
      addToast(`Requesting devnet faucet airdrop for ${agentId}…`, "info")
      const faucetRes  = await fetch(`/api/agents/${agentId}/faucet`, { method: "POST" })
      const faucetJson = (await faucetRes.json()) as FundResult
      if (faucetJson.ok) {
        addToast(`Faucet airdrop confirmed — ${faucetJson.newBalanceSOL?.toFixed(4)} SOL`, "success", faucetJson.explorerUrl)
        await fetchAgents()
        return
      }
      // Faucet returned an error response — notify and fall through
      addToast(`Devnet faucet unavailable: ${faucetJson.error ?? "rate-limited"}. Falling back to master wallet…`, "error")
    } catch {
      addToast("Devnet faucet unreachable. Falling back to master wallet…", "error")
    }

    // Step 2 — fallback: fund from master wallet
    try {
      const masterRes  = await fetch(`/api/agents/${agentId}/airdrop`, { method: "POST" })
      const masterJson = (await masterRes.json()) as FundResult
      if (!masterJson.ok) throw new Error(masterJson.error ?? "Master wallet funding failed")
      addToast(`Master wallet funded ${agentId} — ${masterJson.newBalanceSOL?.toFixed(4)} SOL`, "success", masterJson.explorerUrl)
      await fetchAgents()
    } catch (err) {
      addToast(err instanceof Error ? err.message : "Funding failed", "error")
    } finally {
  ...
  const handleToggleLoop = useCallback(async (agentId: string, isRunning: boolean) => {
    try {
      ...
      addToast(
        `${agentId} loop ${action === "start" ? "started — agent is now trading autonomously" : "stopped"}`,
        action === "start" ? "success" : "info"
      )
      // Eagerly re-fetch so trade counts appear without waiting for the next poll
      if (action === "start") {
      ...
      <div style={{ background: "rgb(6,6,10)", minHeight: "100vh", position: "relative" }}>
        ...
        <main style={{ maxWidth: "1200px", margin: "0 auto", padding: "7rem 1.5rem 5rem", position: "relative", zIndex: 1 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <div style={{
                ...
                <span style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.75rem", letterSpacing: "0.04em" }}>
                  {loading    ? "Connecting…"
                  : isIniting ? "Initializing wallets…"
                  : isInit    ? `${running}/${agents.length} agents live`
                  : "Starting up…"}
                </span>
              </div>
            </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: "1rem" }}>
                {agents.map((a) => (
                  <div key={a.id} style={{
                    ...
                    <span style={{ color: "#9945FF", fontFamily: "'Space Mono', monospace", fontSize: "0.7rem", minWidth: 120 }}>
                      {a.id}
                    </span>
                    <span style={{ color: "rgba(255,255,255,0.5)", fontFamily: "'Space Mono', monospace", fontSize: "0.7rem", flex: 1 }}>
                      {a.publicKey}
                    </span>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: "1.25rem" }}>
            ...
            <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: "1.2rem", fontWeight: 800, letterSpacing: "-0.03em", color: "#fff" }}>
              Agent Wallets
            </h2>
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: "1.5rem" }}>
              ...
              <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: "1.1rem", fontWeight: 800, letterSpacing: "-0.03em", color: "#fff" }}>
                How this works
              </h2>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1.5rem" }}>
              {[
                ...
                <div key={title}>
                  ...
                  <p style={{ color: "#fff", fontSize: "0.85rem", fontWeight: 700, fontFamily: "'Syne', sans-serif", marginBottom: 5 }}>
                    {title}
                  </p>
                  <p style={{ color: "rgba(255,255,255,0.35)", fontSize: "0.75rem", lineHeight: 1.75, fontFamily: "'Space Mono', monospace" }}>
                    {body}
                  </p>
                </div>
              ))}
